...
public class Main {
    ...
    private static void MainFrame() {
        ...
        JPanel cryptionButton = new JPanel();
        ...
        cryptionButton.add(encryptionButton);
        cryptionButton.add(decryptionButton);

        mainFrame.add(functionButton, BorderLayout.EAST);
        mainFrame.add(scrollPane, BorderLayout.NORTH);
        mainFrame.add(cryptionButton, BorderLayout.WEST);
    ...
    private static void createPassword() {
        ...
        newSaveButton.addActionListener(e -> {
            try {
                ...
                } else if (checkProductName(websiteNameTextField.getText()) && checkProductName(userNameTextField.getText())) {
                    JOptionPane.showMessageDialog(mainFrame, "Invalid name. Please enter a valid name.");
                } else if (checkProductName(websiteNameTextField.getText())) {
                    JOptionPane.showMessageDialog(mainFrame, "Invalid website name. Please enter a valid name.");
                } else if (checkProductName(userNameTextField.getText())) {
                    JOptionPane.showMessageDialog(mainFrame, "Invalid username. Please enter a valid name.");
                } else {
    ...
    private static void editPassword() {
        ...
        if (selectedRows.length != 1) {
            JOptionPane.showMessageDialog(mainFrame, "Please select only one row to edit.");
            return;
        }
    ...
    private static boolean checkProductName(String name) {
        // Check if the name contains any alphabetical characters
        boolean containsLetters = name.matches(".*[a-zA-Z].*");

        // Check if the name is not pure numbers or pure symbols
        boolean notPureNumbers = !name.matches("^\\d+$");
        boolean notPureSymbols = !name.matches("^\\W+$");

        return !containsLetters || (!notPureNumbers && !notPureSymbols);
    }
    ...
    private static void deletePassword() {
        ...
        if (selectedRows.length == 0) {
            JOptionPane.showMessageDialog(mainFrame, "Please select at least one row to delete.");
            return;
        }

        String adminInput = JOptionPane.showInputDialog(mainFrame, "Enter admin password:");
        if (adminInput != null && adminInput.equals(adminPassword)) {
            int option = JOptionPane.showConfirmDialog(mainFrame, "Are you sure you want to delete the selected passwords?", "Confirm Delete", JOptionPane.YES_NO_OPTION);
